...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    let upnp = chars
        ...
        .expect("UPNP char not found");
    let avdtp = chars
        .iter()
        .find(|c| c.uuid == AVDTP_UUID)
        .expect("AVDTP char not found");
    ...
    let nus_tx = chars
        .iter()
        .find(|c| c.uuid == Uuid::from_u128(0x6e400002_b5a3_f393_e0a9_e50e24dcca9e));
    ...
    if let Some(tx) = nus_tx {
        device
            .subscribe(
                &chars
                    .iter()
                    .find(|c| c.uuid == Uuid::from_u128(0x6e400003_b5a3_f393_e0a9_e50e24dcca9e))
                    .unwrap(),
            )
            .await
            .ok();
