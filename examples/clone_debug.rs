...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    let commands: Vec<(&str, Vec<u8>)> = vec![
        ...
        (
            "Get Serial",
            build_xiaomi_packet(&[0x03, 0x20, 0x01, 0x10, 0x16]),
        ),
        // Read Firmware Version: Dev=0x20, Cmd=0x01, Attr=0x1A, Len=0x02
        (
            "Get Version",
            build_xiaomi_packet(&[0x03, 0x20, 0x01, 0x1A, 0x02]),
        ),
        // Read Battery %: Dev=0x20, Cmd=0x01, Attr=0x32, Len=0x02
        (
            "Get Battery",
            build_xiaomi_packet(&[0x03, 0x20, 0x01, 0x32, 0x02]),
        ),
    ...
    if let Some(upnp_char) = upnp {
        ...
        for (name, cmd) in &commands {
            ...
            if let Err(e) = device
                .write(upnp_char, cmd, WriteType::WithoutResponse)
                .await
            {
                println!("  Write failed: {}", e);
                continue;
    ...
    if let Some(avdtp_char) = avdtp {
        ...
        for (name, cmd) in &commands {
            ...
            if let Err(e) = device
                .write(avdtp_char, cmd, WriteType::WithoutResponse)
                .await
            {
                println!("  Write failed: {}", e);
                continue;
