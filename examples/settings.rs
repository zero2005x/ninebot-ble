...
async fn main() -> Result<()> {
    ...
    tracing::info!(
        "  Tail light enabled: {:?}, Switching on",
        session.tail_light().await?
    );
    ...
    tracing::info!(
        "  Tail light enabled: {:?}, Switching off",
        session.tail_light().await?
    );
