...
impl MiProtocol {
    pub async fn new(device: &Peripheral) -> Result<Self> {
        ...
        let stream: Pin<Box<dyn Stream<Item = ValueNotification>>> =
            device
                .notifications()
                .await
                .with_context(|| "Could not load notifications stream".to_string())?;
    ...
    pub async fn wait_for_notification(&mut self) -> Result<ValueNotification> {
        self.wait_for_notification_with_timeout(Duration::from_secs(10))
            .await
    }
    ...
    pub async fn write(&self, reg: &Registers, command: MiCommands) -> Result<bool> {
        ...
        self.device
            .write(channel, &command.to_bytes(), WriteType::WithoutResponse)
            .await
    ...
    pub async fn write_mi_parcel(&self, reg: &Registers, data: &[u8]) -> Result<bool> {
        ...
        for chunk in data.chunks(MI_CHUNK_SIZE) {
            ...
            self.device
                .write(channel, &buffer, WriteType::WithoutResponse)
                .await
                .with_context(|| {
                    format!(
                        "Could not write mi chunk: {} for channel: {:?}",
                        chunk_index, channel
                    )
                })?;
            chunk_index += 1;
...
async fn find_characteristic(
    ...
    device
        .discover_services()
        .await
        .with_context(|| "Could not enable discovering devices".to_string())?;
...
async fn setup_channels(
    ...
    let avdtp = find_characteristic(
        device,
        Registers::AUTH.to_uuid(),
        Registers::AVDTP.to_uuid(),
    )
    .await?;
    let upnp =
        find_characteristic(device, Registers::AUTH.to_uuid(), Registers::UPNP.to_uuid()).await?;

    // UART channels
    tracing::debug!("Setting up UART channels");
    let tx =
        find_characteristic(device, Registers::UART.to_uuid(), Registers::TX.to_uuid()).await?;
    let rx =
        find_characteristic(device, Registers::UART.to_uuid(), Registers::RX.to_uuid()).await?;

    tracing::debug!("Enabling notify for AVDTP");
    device
        .subscribe(&avdtp)
        .await
        .with_context(|| "Could not subscribe to scooter AVDTP notifications".to_string())?;

    tracing::debug!("Enabling notify for UPNP");
    device
        .subscribe(&upnp)
        .await
        .with_context(|| "Could not subscribe to scooter UPNP notifications".to_string())?;

    tracing::debug!("Enabling notify for RX");
    device
        .subscribe(&rx)
        .await
        .with_context(|| "Could not subscribe to scooter RX notifications".to_string())?;
