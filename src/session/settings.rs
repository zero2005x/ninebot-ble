use super::commands::{Attribute, Direction, ReadWrite, ScooterCommand};
...
impl MiSession {
    pub async fn supplementary_info(&mut self) -> Result<SupplementaryInfo> {
        ...
        SupplementaryInfo::try_from(payload)
    }
    ...
    pub async fn tail_light(&mut self) -> Result<TailLight> {
        ...
        Ok(TailLight::from(payload.pop_u16()?))
    }
    ...
    /// Set KERS (brake energy recovery) level. 0=off, 1=weak, 2=medium, 3=strong
    pub async fn set_kers(&mut self, level: u8) -> Result<()> {
        tracing::debug!("Setting KERS level: {}", level);
        let payload = vec![level, 0x00];
        self.send(&ScooterCommand {
            direction: Direction::MasterToMotor,
            read_write: ReadWrite::Write,
