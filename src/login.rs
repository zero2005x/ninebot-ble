...
use crate::mi_crypto::{calc_login_did, gen_rand_key, AuthToken, LoginKeychain, RandKey};
...
#[derive(Error, Debug)]
pub enum LoginError {
    #[error("Failed at creating session for scooter connection")]
    //TODO: less retarded error message
    LoginFailed,
...
impl LoginRequest {
    ...
    async fn send_key(&mut self) -> Result<bool> {
        self.protocol
            .write(&Registers::UPNP, MiCommands::CMD_LOGIN)
            .await?;
        self.protocol
            .write(&Registers::AVDTP, MiCommands::CMD_SEND_KEY)
            .await?;

        self.protocol.wait_for_scooter_to_receive_data().await?;
        self.protocol
            .write_mi_parcel(&Registers::AVDTP, &self.rand_key)
            .await?;
    ...
    async fn validate_remote_key_and_send_did(&mut self) -> Result<bool, LoginError> {
        ...
        let (info, expected_remote_info, keys) =
            calc_login_did(rand_key, remote_key, &self.auth_token);
        if remote_info == expected_remote_info {
            tracing::debug!("Remote info is as expected, sending did");

            self.protocol
                .write(&Registers::AVDTP, MiCommands::CMD_SEND_INFO)
                .await?;

            self.protocol.wait_for_scooter_to_receive_data().await?;
            self.protocol
                .write_mi_parcel(&Registers::AVDTP, &info)
                .await?;
            self.protocol.wait_for_scooter_to_ack_data().await?;
            self.keys = Some(keys);
