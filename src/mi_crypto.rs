...
fn encrypt_did(key: &[u8], did: &[u8]) -> Vec<u8> {
    ...
    aes_ccm
        .encrypt(nonce, Payload { msg: did, aad })
...
pub fn calc_did(
    ...
    tracing::debug!(
        "Calculating did with remote key: {:?}",
        key_bytes.hex_dump()
    );

    let remote_public_key =
        PublicKey::from_sec1_bytes(key_bytes).expect("Key sent by scooter is invalid");
...
pub fn encrypt_uart(
    ...
    nonce.extend(std::iter::repeat_n(0, 4));
...
pub fn decrypt_uart(encryption_key: &EncryptionKey, msg: &[u8]) -> Result<Vec<u8>, MiCryptoError> {
    ...
    nonce.extend(std::iter::repeat_n(0, 4));
    nonce.extend_from_slice(it);
    nonce.extend(std::iter::repeat_n(0, 2));
    tracing::debug!("  nonce: {:?}", nonce.hex_dump());
    ...
    let data = match aes_ccm.decrypt(
        nonce,
        Payload {
            msg: ct,
