...
impl RegistrationRequest {
    ...
    async fn read_remote_info(&mut self) -> Result<bool> {
        self.protocol
            .write(&Registers::UPNP, MiCommands::CMD_GET_INFO)
            .await?;
    ...
    async fn send_public_key(&mut self) -> Result<bool, RegistrationError> {
        self.protocol
            .write(&Registers::UPNP, MiCommands::CMD_SET_KEY)
            .await?;
        self.protocol
            .write(&Registers::AVDTP, MiCommands::CMD_SEND_DATA)
            .await?;
        ...
        match MiCommands::try_from(notification) {
            Ok(MiCommands::RCV_RDY) => {
                ...
                tracing::debug!(
                    "-> Mi ready to receive key, uploading my public key: {:?}",
                    public_key_bytes.as_bytes().hex_dump()
                );
                self.protocol
                    .write_mi_parcel(&Registers::AVDTP, &public_key_bytes.as_bytes()[1..])
                    .await?;
            }
            Ok(other) => {
                tracing::debug!("Could not match: {:?}", other);
                return Err(RegistrationError::Other(anyhow!(
                    "Scooter responded with: {:?} instead RCV_RDY",
                    other
                )));
            }
            Err(err) => return Err(RegistrationError::Other(anyhow!(err))),
        }
        ...
        Err(RegistrationError::Other(anyhow!(
    ...
    async fn send_did(&mut self) -> Result<bool> {
        ...
        let (did_ct, token) =
            mi_crypto::calc_did(&self.my_secret_key, &remote_key_bytes, remote_info);

        self.token = Some(token);
        self.protocol
            .write(&Registers::AVDTP, MiCommands::CMD_SEND_DID)
            .await?;
        ...
        loop {
            match self.protocol.next_mi_response().await {
                Some(MiCommands::RCV_RDY) => {
                    ...
                    self.protocol
                        .write_mi_parcel(&Registers::AVDTP, &did_ct)
                        .await?;
                }
    ...
    async fn perform_auth(&mut self) -> Result<bool, RegistrationError> {
        self.protocol
            .write(&Registers::UPNP, MiCommands::CMD_AUTH)
            .await?;
        match self.protocol.next_mi_response().await {
