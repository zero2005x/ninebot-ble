# ninebot-ble

![MIT license](https://img.shields.io/github/license/zero2005x/ninebot-ble)
![Crates.io version](https://img.shields.io/crates/v/ninebot-ble)

透過 BLE（藍牙低功耗）與九號/小米電動滑板車（M365、Mi Pro 等）通訊的輕量級 Rust 函式庫。

> 📖 **[English Documentation](../README.md)**

## 功能特色

- 🔍 **掃描器** - 尋找附近的 M365 滑板車
- 🔐 **註冊** - 使用 ECDH 金鑰交換與滑板車配對
- 🔑 **登入** - 使用已儲存的 Token 認證
- 📊 **讀取資料** - 電池、速度、里程、溫度等
- ⚙️ **設定** - 控制定速巡航、尾燈、動能回收等級
- 🎮 **互動控制器** - 即時監控與控制

## 支援平台

本函式庫使用 [btleplug](https://crates.io/crates/btleplug) 提供跨平台 BLE 支援：

- Windows 10/11
- macOS
- Linux
- iOS

## 支援的滑板車型號

| 型號          | 狀態    |
| ------------- | ------- |
| 小米 M365     | ✅ 支援 |
| 小米 Mi 1S    | ✅ 支援 |
| 小米 Mi Pro   | ✅ 支援 |
| 小米 Mi Pro 2 | ✅ 支援 |
| 小米 Mi Pro 3 | ✅ 支援 |
| 副廠控制器    | ⚠️ 部分 |

## 快速開始

### 安裝

在 `Cargo.toml` 中添加：

```toml
[dependencies]
ninebot-ble = "0.1"
```

### 使用範例

#### 1. 尋找 MAC 位址

```bash
cargo run --example scanner
```

輸出：

```
INFO scanner: Found scooter nearby: MIScooter7353 with mac: D5:01:45:37:ED:FD
```

#### 2. 註冊（僅首次）

⚠️ **警告：** 註冊會解除與其他應用程式的配對！

```bash
cargo run --example register D5:01:45:37:ED:FD
```

這會將認證 Token 儲存到 `.mi-token` 檔案。

#### 3. 登入

```bash
cargo run --example login D5:01:45:37:ED:FD
```

#### 4. 讀取資訊

```bash
cargo run --example about D5:01:45:37:ED:FD
```

輸出：

```
Battery info: BatteryInfo { capacity: 7392, percent: 63, voltage: 36.74 }
Serial number: 26354/00467353
Motor info: MotorInfo { speed_kmh: 0, total_distance_m: 1306083 }
```

#### 5. 互動控制器

```bash
cargo run --example controller D5:01:45:37:ED:FD
```

## BLE 協議

### 服務與特徵值

| UUID           | 名稱     | 描述                    |
| -------------- | -------- | ----------------------- |
| `FE95`         | 認證服務 | 小米認證服務            |
| `0x0010`       | UPNP     | 命令控制                |
| `0x0019`       | AVDTP    | 資料交換                |
| `6e400002-...` | TX       | 寫入（客戶端 → 滑板車） |
| `6e400003-...` | RX       | 通知（滑板車 → 客戶端） |

### UART 訊框格式

```
+-----+-----+-----+-----+-----+-----+-------+------+------+
| 0x55| 0xAA|  L  |  D  |  T  |  C  |  ...  | CK0  | CK1  |
+-----+-----+-----+-----+-----+-----+-------+------+------+
  標頭      長度  裝置  命令  屬性   資料    校驗碼
```

| 欄位        | 描述                                         |
| ----------- | -------------------------------------------- |
| `0x55 0xAA` | 訊框標頭                                     |
| `L`         | 長度 = 資料長度 + 2                          |
| `D`         | 裝置：`0x20`=主機 → 馬達，`0x22`=主機 → 電池 |
| `T`         | 類型：`0x01`=讀取，`0x03`=寫入               |
| `CK0, CK1`  | 校驗碼 = (從 L 開始的位元組總和) XOR 0xFFFF  |

## 加密流程

### 註冊（一次性）

```
客戶端                              滑板車
  │                                   │
  │──── CMD_GET_INFO ────────────────►│
  │◄──── 遠端資訊 ────────────────────│
  │                                   │
  │  產生 ECDH 金鑰對 (P-256)         │
  │──── 我的公鑰 ────────────────────►│
  │◄──── 滑板車公鑰 ──────────────────│
  │                                   │
  │  計算：                           │
  │  - 共享密鑰 (ECDH)                │
  │  - Token, BindKey (HKDF-SHA256)   │
  │  - DID_CT (AES-CCM 加密)          │
  │                                   │
  │──── DID_CT ──────────────────────►│
  │◄──── AUTH_OK ─────────────────────│
  │                                   │
  │  儲存 Token (12 位元組)           │
```

### 登入（每次連線）

```
客戶端                              滑板車
  │                                   │
  │──── CMD_LOGIN ───────────────────►│
  │──── 隨機金鑰 (16 位元組) ────────►│
  │◄──── 遠端隨機金鑰 ────────────────│
  │◄──── 遠端資訊 (32 位元組) ────────│
  │                                   │
  │  派生金鑰 (HKDF-SHA256)：         │
  │  - DevKey, AppKey (AES-128)       │
  │  - DevIV, AppIV (各 4 位元組)     │
  │                                   │
  │  驗證：HMAC(DevKey, salt)         │
  │                                   │
  │──── DID 資訊 ────────────────────►│
  │◄──── LOGIN_OK ────────────────────│
```

### UART 加密 (AES-128-CCM)

```
加密（客戶端 → 滑板車）：
  nonce = AppIV + "0000" + 計數器
  密文 = AES-CCM(AppKey, 訊息, nonce)

解密（滑板車 → 客戶端）：
  nonce = DevIV + "0000" + 計數器
  明文 = AES-CCM(DevKey, 密文, nonce)
```

## 可讀取的資料

| 類別 | 資料                                     |
| ---- | ---------------------------------------- |
| 馬達 | 速度、平均速度、里程、運行時間、溫度     |
| 電池 | 電壓、電流、容量、百分比、電芯電壓、溫度 |
| 設定 | 定速巡航、尾燈、動能回收等級             |
| 資訊 | 序號、PIN 碼、韌體版本                   |

## API 參考

### 掃描器

```rust
use ninebot_ble::scanner::ScooterScanner;

let scanner = ScooterScanner::new().await?;
let scooters = scanner.scooters().await;
```

### 註冊

```rust
use ninebot_ble::register::MiRegister;

let device = scanner.connect_to("D5:01:45:37:ED:FD").await?;
let mut register = MiRegister::new(&device).await?;
let token = register.register().await?;
```

### 登入與會話

```rust
use ninebot_ble::login::MiLogin;

let mut login = MiLogin::new(&device, &token).await?;
let session = login.start().await?;

// 讀取資料
let battery = session.battery_info().await?;
let motor = session.motor_info().await?;
```

## 專案結構

```
m365/
├── src/
│   ├── lib.rs           # 函式庫入口
│   ├── scanner.rs       # BLE 裝置掃描
│   ├── connection.rs    # BLE 連線管理
│   ├── protocol.rs      # MiAuth 協議實作
│   ├── register.rs      # 裝置註冊
│   ├── login.rs         # 認證
│   ├── mi_crypto.rs     # 加密操作
│   ├── consts.rs        # 常數
│   └── session/         # 會話命令
│       ├── mi_session.rs
│       ├── battery.rs
│       ├── info.rs
│       ├── settings.rs
│       └── commands.rs
├── examples/
│   ├── scanner.rs       # 尋找滑板車
│   ├── register.rs      # 註冊滑板車
│   ├── login.rs         # 登入範例
│   ├── about.rs         # 讀取所有資訊
│   ├── settings.rs      # 變更設定
│   └── controller.rs    # 互動控制器
└── tests/
    ├── crypto_test.rs
    ├── motor_info_test.rs
    └── uart_test.rs
```

## 授權

本專案採用 MIT 授權 - 詳見 [LICENSE.md](../LICENSE.md) 檔案。

## 致謝

- 基於 [CamiAlfa's M365-BLE-PROTOCOL](https://github.com/CamiAlfa/M365-BLE-PROTOCOL) 的研究
- 使用 [btleplug](https://crates.io/crates/btleplug) 提供跨平台 BLE 支援
